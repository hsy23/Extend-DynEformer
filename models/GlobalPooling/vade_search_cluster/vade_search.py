...
class VaDE(nn.Module):
    ...
    def train_search(self, pre_epoch=10):
        Loss = nn.MSELoss()
        ...
        for best_n in self.args.nClusters:
            gmm = GaussianMixture(n_components=best_n, covariance_type='diag')
            try:
                gmm_model = gmm.fit(Z.reshape(-1, 10))
                keep_n.append(best_n)
            except:
                continue

            aic = gmm_model.aic(Z.reshape(-1, 10))
            bic = gmm_model.bic(Z.reshape(-1, 10))
            print('aic:{}, bic:{}'.format(aic, bic))
            aic_l.append(aic)
            bic_l.append(bic)

            all_pre_res.append(gmm_model.predict(Z.reshape(-1, 10)))
        ...
        print('keep_n:', keep_n)
        pickle.dump(aic_l, open('./seasonal_aic_l_800.pkl', 'wb'))
        pickle.dump(bic_l, open('./seasonal_bic_l_800.pkl', 'wb'))
        pickle.dump(all_pre_res, open('./seasonal_pre_res_800.pkl', 'wb'))
        return X_train_all_np, keep_n, all_pre_res, aic_l, bic_l
