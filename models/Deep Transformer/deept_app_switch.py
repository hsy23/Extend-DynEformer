...
def reference(X, y, args):
    ...
    xscaler, yscaler = pickle.load(open('8_scalers.pkl', 'rb'))  # MinMax Scaler saved from ECW
    Xte = xscaler.transform(Xte.reshape(-1, num_features)).reshape(num_ts, -1, num_features, args.step)
