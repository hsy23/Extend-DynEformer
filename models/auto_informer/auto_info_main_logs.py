import datetime
import sys
sys.path.append("/data/shaoyuan/TPAMI-DynEformer")
...
# model define
parser.add_argument('--model_type', type=str, default='Informer', help='model type')
...
def get_ppio(args):
    X = np.load(open(r"/data/shaoyuan/TPAMI-DynEformer/data/ECW_0809_big.npy", 'rb'), allow_pickle=True)
    y = X[:, :, 0]
    ...
    Xte_loader = DataLoader(PPIO_Dataset(Xte, args.seq_len, args.label_len, args.pred_len), batch_size = args.batch_size)
    Xtr_loader = DataLoader(PPIO_Dataset(Xtr, args.seq_len, args.label_len, args.pred_len), batch_size = args.batch_size)
...
print(args.model_type, args.pred_len)

log_file_name = "logs/auto_info_main_log_" + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + ".txt"
...
Xtr_loader, Xte_loader, yscaler = get_ppio(args)
min_loss = 1000
best_model = None
...
for epoch in range(epochs):
    ...
    with open(log_file_name, "a") as log_file:
        log_file.write(f'epoch {epoch}, train loss: {train_loss[-1]}, test loss: {test_loss[-1]}, mse: {test_mse[-1]}, mape: {test_mape[-1]}, mae: {test_mae[-1]}' + "\n")

print(args.model_type, args.pred_len)
print(np.min(test_mse), np.min(test_mape), np.min(test_mae))

with open(log_file_name, "a") as log_file:
    log_file.write(np.min(test_mse), np.min(test_mape), np.min(test_mae))
