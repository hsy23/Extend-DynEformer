...
import argparse
import pickle
from dyneformer_origin import DynEformer
...
def train(X, y, args):
    ...
    cn = 550
    print('cn: {}'.format(cn))
    with open(r'../GlobalPooling/pools/global_pool_c{}_s48_s12.pkl'.format(cn), 'rb') as f:
    ...
    Xtr, ytr, Xte, yte = train_test_split(X, y, args.train_ratio)
    ...
    if args.save_scaler:
        pickle.dump([xscaler, yscaler], open('saved_models/dyne_data_scaler_tmp.pkl', 'wb'))
    ...
    for epoch in range(args.num_epoches):
        ...
        if args.save_model:
            if test_loss[-1] < min_loss:
                ...
                torch.save(model, 'saved_model/DynEformer_best_n{}_drop01.pt'.format(cn))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_scaler", "-sas", type=bool, default=False)
    ...
    if args.run_test:
        ...
        # DynEformer_origin-gp:0.0807542999403596 0.15957949686148326 99