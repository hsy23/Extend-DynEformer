...
import argparse
import pickle
from dyneformer_pro import DynEformer
...
def get_ppio(X, y, batch_size, args):
    ...
    Xtr_loader = DataLoader(PPIO_Dataset(Xtr, args.enc_len, args.label_len, args.pred_len, args.step), batch_size=batch_size)
    Xte_loader = DataLoader(PPIO_Dataset(Xte, args.enc_len, args.label_len, args.pred_len, args.step), batch_size=batch_size)
...
def train(X, y, args):
    ...
    with open(r'../GlobalPooling/pools/pools_0809_sx_s12_big/global_pool_c{}_s{}_s12.pkl'.format(args.gp_len, args.gp_seq_len), 'rb') as f:
        globalPool_pool = pickle.load(f)
    sFeature = torch.tensor(np.stack(globalPool_pool.seasonal_pool).T).float().to(device)
    globalPool = sFeature

    args.gp_len = globalPool.shape[-1]
    args.gp_seq_len = globalPool.shape[0]
    print('cn: {}'.format(args.gp_len))

    model = DynEformer(args)
    ...
    for epoch in range(args.num_epoches):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(Xtr_loader):
            ...
            outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_x_static, globalPool)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_len", "-ol", type=int, default=24*3)  # 预测长度
    ...
    parser.add_argument("--gp_len", type=int, default=600)  # 全局池类别数
    parser.add_argument("--gp_seq_len", type=int, default=72)  # 全局池序列长度
