...
class EncoderLayer(torch.nn.Module):
    def __init__(self, dim_val, dim_attn, dim_feedforward, dropout, n_heads=1, activation='relu'):
        super(EncoderLayer, self).__init__()
        self.attn = MultiHeadAttentionBlock(dim_val, dim_attn, n_heads)
        self.fc1 = nn.Linear(dim_val, dim_feedforward)
        self.fc2 = nn.Linear(dim_feedforward, dim_val)
    ...
    def forward(self, x):
        ...
        a = self.fc2(self.dropout(self.activation(self.fc1(x))))
...
class DynEformer(torch.nn.Module):
    def __init__(self, dim_val, dim_attn, dim_feedforward, input_size, enc_seq_len, dec_seq_len, out_seq_len,
                 n_decoder_layers=1, n_encoder_layers=1, n_heads=1, dropout=0.5, activation='relu',
                 time_feature_len=150, dim_static=12, if_padding=True):
        ...
        for i in range(n_encoder_layers):
            self.encs.append(EncoderLayer(dim_val, dim_attn, dim_feedforward, dropout, n_heads, activation))
        ...
        for i in range(n_decoder_layers):
            self.decs.append(DecoderLayer(dim_val, dim_attn, dim_feedforward, dropout, n_heads, activation))
    ...
    def forward(self, x, time_feature=None, static_context=None):
        # x = x.transpose(0, 1)
        # encoder
        ...
        if time_feature!=None:
            ...
            if self.if_padding:
                ...
                dec_input[:, -self.out_seq_len:, 0] = 0
        else:
            for enc in self.encs[1:]:
                e = enc(e)
            dec_input[:, -self.out_seq_len:, 0] = 0
